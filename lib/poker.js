...
POKER.Hand = function(cards) {
    ...
    this.getHandValue = function () {
        if (this.numSameSuits() === 5 && this.numConnected() === 5) {
            return [POKER.HAND_TYPE.STRAIGHT_FLUSH].concat(this.getRankByOccurance(1));
        };
        if (this.numOfAKind()[0] === 4) {
            return [POKER.HAND_TYPE.QUADS].concat(this.getRankByOccurance(4), this.getRankByOccurance(1));
        };
        if (equivIntArrays(this.numOfAKind(), [3,2])) {
            return [POKER.HAND_TYPE.FULL_HOUSE].concat(this.getRankByOccurance(3), this.getRankByOccurance(2));
        };
        if (this.numSameSuits() === 5 && this.numConnected() < 5) {
            return [POKER.HAND_TYPE.FLUSH].concat(this.getRankByOccurance(1));
        };
        if (this.numConnected() === 5 && this.numSameSuits() < 5) {
            return [POKER.HAND_TYPE.STRAIGHT].concat(this.getRankByOccurance(1));
        };
        if (equivIntArrays(this.numOfAKind(), [3,1,1])) {
            return [POKER.HAND_TYPE.TRIPS].concat(this.getRankByOccurance(3), this.getRankByOccurance(1));
        };
        if (equivIntArrays(this.numOfAKind(), [2,2,1])) {
            return [POKER.HAND_TYPE.TWO_PAIR].concat(this.getRankByOccurance(2), this.getRankByOccurance(1));
        };
        if (equivIntArrays(this.numOfAKind(), [2,1,1,1])) {
            return [POKER.HAND_TYPE.PAIR].concat(this.getRankByOccurance(2), this.getRankByOccurance(1));
        };
